/*----------------------------------------------------------------------------------------------/
 ...
 Objetivo..: Interface Exportaá∆o Clientes SFA
 Data......: 26/02/2019
 Autor.....: RogÇrio Dias
 Vers∆o....: 1.000.000
-----------------------------------------------------------------------------------------------*/
...
PROCEDURE piGravaTTEmitente:
    ...
    DEFINE VARIABLE cTipoCredito AS CHARACTER   NO-UNDO INITIAL "Normal,Autom†tico,S¢ Imp Ped, Suspenso, Pg Ö Vista".
    ...
    IF AVAIL emitente THEN DO:
        ...
        IF AVAIL es-emitente THEN do:
            IF es-emitente.regime-trib = ? THEN
                ASSIGN tt_emitente.RegimeTributario = "N∆o Classificado".
            ELSE ASSIGN tt_emitente.RegimeTributario = es-emitente.regime-trib.
        END.
        ELSE ASSIGN tt_emitente.RegimeTributario = "N∆o Classificado".
...
PROCEDURE piGravaTTEnderecoList:
    ...
    IF AVAIL emitente THEN DO:
        ...
        FOR EACH loc-entr WHERE loc-entr.nome-abrev = emitente.nome-abrev NO-LOCK:
            ...
            IF loc-entr.cod-entrega = "Padr∆o" THEN
