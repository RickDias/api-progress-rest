...
*******************************************************************************/
{include/i-prgvrs.i esint003-G01 12.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cd-sistema c_des-sistema bt-ok ~
bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cd-sistema c_des-sistema 
...
DEFINE VARIABLE c_des-sistema AS CHARACTER FORMAT "x(40)":U 
     LABEL "Descri‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 37.14 BY 1.25 NO-UNDO.

DEFINE VARIABLE i_cd-sistema AS INTEGER FORMAT ">,>>>,>>9":U 
...
DEFINE FRAME d-vapara
     i_cd-sistema AT ROW 1.25 COL 20.86 COLON-ALIGNED WIDGET-ID 2
     c_des-sistema AT ROW 2.63 COL 20.86 COLON-ALIGNED WIDGET-ID 4
...
DO:
    IF INPUT FRAME {&FRAME-NAME} i_cd-sistema <> 0
    THEN FIND FIRST es-api-app NO-LOCK
         WHERE es-api-app.cd-sistema  EQ INPUT FRAME {&FRAME-NAME} i_cd-sistema
         NO-ERROR.
    
    IF INPUT FRAME {&FRAME-NAME} c_des-sistema <> ""
    THEN FIND FIRST es-api-app NO-LOCK
         WHERE es-api-app.des-sistema EQ INPUT FRAME {&FRAME-NAME} c_des-sistema 
         NO-ERROR.


    IF NOT AVAILABLE es-api-app 
    THEN DO:
        {utp/ut-table.i mgcam es-api-app 1}
        RUN utp/ut-msgs.p(INPUT "show",
                          INPUT 2,
                          INPUT RETURN-VALUE).
        RETURN NO-APPLY.
    END.
    ASSIGN p-row-tabela = ROWID(es-api-app).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cd-sistema c_des-sistema 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cd-sistema c_des-sistema bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esint003-G01" "12.00.00.000"}
