...
do on stop undo, leave:
    ...
    FOR EACH es-api-param NO-LOCK
       ...
       FIRST es-api-app NO-LOCK
          OF es-api-param:
       FOR EACH sfa-import NO-LOCK
             OF es-api-param
          WHERE sfa-import.data-movto      >= da-periodo-ini
            AND sfa-import.data-movto      <= da-periodo-fim
            AND sfa-import.ind-tipo-trans  =  i-relat:
           PUT
               sfa-import.cd-tipo-integr                                                 CHR(9)
               es-api-param.des-tipo-integr                                              CHR(9)
               es-api-app.cd-sistema                                                     CHR(9)
               es-api-app.des-sistema                                                    CHR(9)
               sfa-import.chave                                                          CHR(9)
               ENTRY(sfa-import.cod-status,"Integrado,Nao Integrado")                    CHR(9)
               sfa-import.data-inicio                                                    CHR(9)
               sfa-import.data-fim                                                       CHR(9)
               sfa-import.data-movto                                                     CHR(9)
               ENTRY(sfa-import.ind-situacao - 1,"Pendente,Em processamento,Processado") CHR(9)
               SKIP.
       END.
    END.
