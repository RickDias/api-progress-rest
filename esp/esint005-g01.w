...
*******************************************************************************/
{include/i-prgvrs.i esint005-G01 12.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button c_token d_validade bt-ok ~
bt-cancela 
&Scoped-Define DISPLAYED-OBJECTS c_token d_validade 
...
DEFINE VARIABLE c_token AS CHARACTER FORMAT "x(78)":U 
     LABEL "In¡cio do Token" 
     VIEW-AS FILL-IN 
     SIZE 108.57 BY 1.25
     FONT 1 NO-UNDO.

DEFINE VARIABLE d_validade AS DATE FORMAT "99/99/9999":U 
     LABEL "Validade" 
     VIEW-AS FILL-IN 
     SIZE 10 BY 1.25 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_token AT ROW 1.25 COL 10.86 COLON-ALIGNED WIDGET-ID 2
     d_validade AT ROW 2.58 COL 10.86 COLON-ALIGNED WIDGET-ID 4
...
DO:
    IF INPUT FRAME {&FRAME-NAME} c_token > ""
    THEN DO:
       FIND FIRST es-api-token-apigee NO-LOCK
            WHERE es-api-token-apigee.Token BEGINS INPUT FRAME {&FRAME-NAME} c_token
            NO-ERROR.
    END.
    IF INPUT FRAME {&FRAME-NAME} d_validade <> ?
    THEN DO:
       FIND FIRST es-api-token-apigee NO-LOCK
            WHERE es-api-token-apigee.Validade EQ INPUT FRAME {&FRAME-NAME} d_validade 
            NO-ERROR.
    END.
    
    IF NOT AVAILABLE es-api-token-apigee 
    THEN DO:
        {utp/ut-table.i mgcam es-api-token-apigee 1}
        RUN utp/ut-msgs.p(INPUT "show",
                          INPUT 2,
                          INPUT RETURN-VALUE).
        RETURN NO-APPLY.
    END.
    ASSIGN p-row-tabela = ROWID(es-api-token-apigee).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_token d_validade 
      WITH FRAME d-vapara.
  ENABLE rt-button c_token d_validade bt-ok bt-cancela 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esint005-G01" "12.00.00.000"}
