...
&Scoped-Define ENABLED-OBJECTS rt-button RECT-1 c_valor bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_valor 
...
DEFINE VARIABLE c_valor AS CHARACTER FORMAT "x(16)":U 
     LABEL "Valor" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_valor AT ROW 2.04 COL 23.29 COLON-ALIGNED WIDGET-ID 2
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     rt-button AT ROW 4.25 COL 1
     RECT-1 AT ROW 1.5 COL 4 WIDGET-ID 4
     SPACE(1.13) SKIP(2.01)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V  Para Tipo Pedido de Compra"
...
ON GO OF FRAME d-vapara /* V  Para Tipo Pedido de Compra */
DO:
FIND es-tipo-ped-compra NO-LOCK
    WHERE es-tipo-ped-compra.valor EQ INPUT FRAME {&FRAME-NAME} c_valor NO-ERROR.

IF NOT AVAILABLE es-tipo-ped-compra THEN DO:
    {utp/ut-table.i mgcamaux es-tipo-ped-compra 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(es-tipo-ped-compra).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Tipo Pedido de Compra */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_valor 
      WITH FRAME d-vapara.
  ENABLE rt-button RECT-1 c_valor bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
