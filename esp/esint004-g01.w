...
*******************************************************************************/
{include/i-prgvrs.i esint004-G01 12.00.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button i_cd-tipo-integr c_des-tipo-integr ~
bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cd-tipo-integr c_des-tipo-integr 
...
DEFINE VARIABLE c_des-tipo-integr AS CHARACTER FORMAT "x(40)":U 
     LABEL "Descri‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 37.14 BY 1.25 NO-UNDO.

DEFINE VARIABLE i_cd-tipo-integr AS INTEGER FORMAT ">,>>>,>>9":U INITIAL 0 
     LABEL "Tipo de Integra‡Æo" 
     VIEW-AS FILL-IN 
     SIZE 9 BY 1.25 NO-UNDO.
...
DEFINE FRAME d-vapara
     i_cd-tipo-integr AT ROW 1.25 COL 27 COLON-ALIGNED WIDGET-ID 2
     c_des-tipo-integr AT ROW 2.54 COL 27 COLON-ALIGNED WIDGET-ID 4
...
DO:
    IF INPUT FRAME {&FRAME-NAME} i_cd-tipo-integr <> 0
    THEN DO:
        FIND FIRST es-api-param NO-LOCK
             WHERE es-api-param.cd-tipo-integr  EQ INPUT FRAME {&FRAME-NAME} i_cd-tipo-integr
             NO-ERROR.
    END.
    
    IF INPUT FRAME {&FRAME-NAME} c_des-tipo-integr <> ""
    THEN DO:
        FIND FIRST es-api-param NO-LOCK
             WHERE es-api-param.des-tipo-integr EQ INPUT FRAME {&FRAME-NAME} c_des-tipo-integr 
             NO-ERROR.
    END.

    IF NOT AVAILABLE es-api-param 
    THEN DO:
        {utp/ut-table.i mgcam es-api-param 1}
        RUN utp/ut-msgs.p(INPUT "show",
                          INPUT 2,
                          INPUT RETURN-VALUE).
        RETURN NO-APPLY.
    END.
    ASSIGN p-row-tabela = ROWID(es-api-param).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_cd-tipo-integr c_des-tipo-integr 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cd-tipo-integr c_des-tipo-integr bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "esint004-G01" "12.00.00.000"}
